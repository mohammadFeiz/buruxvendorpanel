...
export default class AIOLogin extends Component {
            ...
            apis: AIOService({
                ...
                getResponse: () => {
                    return {
                        ...
                        async onSubmitUserId({model,mode}) {
                            let result = await onSubmitUserId(model,mode);
                            if (result === false) {
                        ...
                        async onSubmitPassword({model,mode}){
                            let result = await onSubmitPassword(model,mode);
                            return { result }
                        },
                        async onRegister({model,mode}) {
                            let result = await onRegister(model,mode);
                            if(typeof result === 'string'){return {result}}
    ...
    render() {
        ...
        let { registerFields, layout, otpLength, COMPONENT, id, fields, time = 30,methods } = this.props;
        ...
        let html = (
            <LoginForm
                ...
                onSubmitPassword={async (model,mode)=>{
                    return await apis({
                        api:'onSubmitPassword', loading: false, parameter: {model,mode}, name: `ارسال رمز عبور`,
                        callback: ({ token }) => { 
                            if (token) { this.setToken(token, model[mode]) 
                        } }
                    })
                }}
                onSubmitUserId={async (model,mode)=>{
                    let title = {OTPPhoneNumber:'شماره همراه',UserName:'نام کاربری',Email:'آدرس ایمیل',PhoneNumber:'شماره همراه'}[mode];
                    return await apis({
                        api:'onSubmitUserId', loading: false, parameter: {model,mode}, name: `ارسال ${title}`,
                    })
                }}
                onRegister={async (model,mode) => await apis({api:'onRegister', loading: false, parameter: {model,mode}, name: `ثبت نام`})}
            />
        )
...
class LoginForm extends Component {
    ...
    getInitialModel() {
        ...
        let obj = {password:''};
        for(let i = 0; i < methods.length; i++){obj[methods[i]] = userId;}
        return obj;
    }
    getDelta() {
        let { time } = this.state;
    ...
    async onSubmit() {
        ...
        else if(userId){await onSubmitPassword(model,mode);}
    ...
    async onSubmitRegisterModel(){
        ...
        let res = await onRegister(model.register);
        if(typeof res === 'string'){this.setState({userId:this.props.userId});}
    ...
    async onSubmitUserId(callInRecode){
        ...
        let response = await onSubmitUserId(body,mode);
        ...
        else {this.setState({ userId:model[mode], showRegisterForm:!response })}
    }
    getInputs() {
    ...
    title_layout() {
        ...
        return {
            ...
            row: [
                { show: !!showRegisterForm, size: 30, html: <Icon path={mdiChevronRight} size={1} />, align: 'v', onClick: () => this.setState({showRegisterForm:false}) },
                { html, align: 'v' }
            ]
        }
    }
    ...
    changeUserId_layout(){
        ...
        return {
            onClick: () => this.setState({ userId: '', model: this.getInitialModel() }), 
        ...
        for (let i = 0; i < methods.length; i++) {
            ...
            others.push({
                ...
                onClick: () => {
                    ...
                    this.setState({ model: this.getInitialModel(),userId:undefined })
                },
                row: [
                            ...
                            column: [
                                {
                                    ...
                                    column: [
                                        { column: [this.title_layout(), this.subtitle_layout(), { size: 12 }] },
                                        this.form_layout(),
                                        this.submit_layout(),
                                        {
                                            gap:12,
...
class SubmitButton extends Component {
    ...
    render() {
        let { disabled, loading, text = 'ورود' } = this.props;
        ...
        return (
            <>
                <button className='aio-login-submit' disabled={isDisabled} onClick={() => this.onClick()}>
